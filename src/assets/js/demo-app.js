...
window.onload = () => {
  let el = document.getElementById('ac-wrapper'),
    // device element
    device = document.getElementById('ac-device'),
    // the device image wrapper
    trigger = device.childNodes[0],
    ...
    navPrev = nav.querySelector('button:first'),
    navNext = nav.querySelector('button:last'),
  ...
  function init() {
    console.log(trigger);
    // show grid
    trigger.addEventListener('click', showGrid);
  ...
  function navigate(direction) {
    ...
    if (support) {
      ...
      var $nextScreenImg = $(
        '<img src="' + $nextScreen.find('img').attr('src') + '"></img>'
      )
        .css({
