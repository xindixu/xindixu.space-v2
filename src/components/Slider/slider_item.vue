  ...
  methods: {
    ...
    transform() {
      ...
      if (
        ...
        (index === this.pageLength - 2 && data.currentPage === -1)
      ) {
        ...
        index === data.currentPage + 1 ||
        (index === 1 && data.currentPage === this.pageLength)
      ) {
      ...
      // 切换优化
      if (
        ((index === this.pageLength - 1 || index === this.pageLength - 2) &&
          data.currentPage === -1) ||
        ((index === 0 || index === 1) && data.currentPage === this.pageLength)
      ) {
        style['opacity'] = '1';
        if (index === 0) {
          style['z-index'] = '99';
      ...
      style[sData.prefixes.transition + 'Duration'] =
        (sData.animation ? options.speed || 300 : 0) + 'ms';
