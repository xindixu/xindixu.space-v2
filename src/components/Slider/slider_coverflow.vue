  ...
  methods: {
    transform: function(index) {
      ...
      if (this.pages[index].style.height) {
        height =
          this.pages[index].style.height.indexOf('px') === -1
            ? (this.s_data.pageHeight *
                parseFloat(this.pages[index].style.height)) /
              100
            : parseFloat(this.pages[index].style.height);
      } else {
      ...
      let width =
        this.pages[index].style.width.indexOf('px') === -1
          ? (this.s_data.pageWidth *
              parseFloat(this.pages[index].style.width)) /
            100
          : parseFloat(this.pages[index].style.width);
      ...
      let rightWidth =
        offsetWidth +
        this.s_data.deviation +
        width * (1 - this.s_data.widthScalingRatio) +
        'px';
      let centerWidth = offsetWidth + 'px';
      let style = {};
      // 基本
      if (
        index === this.data.currentPage - 1 ||
        (index === this.pages.length - 2 && this.data.currentPage === -1)
      ) {
        ...
        index === this.data.currentPage + 1 ||
        (index === 1 && this.data.currentPage === this.pages.length)
      ) {
      ...
      // 切换优化
      if (
        ((index === this.pages.length - 1 || index === this.pages.length - 2) &&
          this.data.currentPage === -1) ||
        ((index === 0 || index === 1) &&
          this.data.currentPage === this.pages.length)
      ) {
        style['opacity'] = '1';
        if (index === 0) {
          style['z-index'] = '99';
